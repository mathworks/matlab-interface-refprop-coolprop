...
P(1) = getFluidProperty(libLoc, 'P', 'Q', 1, 'T', unit_convert_SI(T_sat_evap, T_unit, 'T', 1), Fluid, 1, 1, 'MASS BASE SI'); % Compressor Inlet : assuming saturation pressure at evaporating temperature
P(2) = getFluidProperty(libLoc, 'P', 'Q', 1, 'T', unit_convert_SI(T_sat_cond, T_unit, 'T', 1), Fluid, 1, 1, 'MASS BASE SI'); % Compressor Outlet: assuming saturation pressure at condensing temperature
...
H(1) = getFluidProperty(libLoc, 'H', 'P', P(1), 'T', unit_convert_SI(T_sat_evap + T_sh, T_unit, 'T', 1), Fluid, 1, 1, 'MASS BASE SI'); % Enthalpy at evaporator exit & compressor inlet
S(1) = getFluidProperty(libLoc, 'S', 'P', P(1), 'H', H(1), Fluid, 1, 1, 'MASS BASE SI'); % Entropy at evaporator exit
H(2) = getFluidProperty(libLoc, 'H', 'P', P(2), 'S', S(1), Fluid, 1, 1, 'MASS BASE SI'); % Enthalpy after isentropic compression
H(2) = (H(2)-H(1))/eta_s+H(1);  % Adjust enthalpy for isentropic efficiency
H(3) = getFluidProperty(libLoc, 'H', 'P', P(3), 'Q', 1, Fluid, 1, 1, 'MASS BASE SI'); % Enthalpy in codenser, saturated vapor
H(4) = getFluidProperty(libLoc, 'H', 'P', P(4), 'Q', 0, Fluid, 1, 1, 'MASS BASE SI'); % Enthalpy in codenser, saturated liquid
H(5) = getFluidProperty(libLoc, 'H', 'P', P(5), 'T', unit_convert_SI(T_sat_cond - T_sc, T_unit, 'T', 1), Fluid, 1, 1, 'MASS BASE SI'); % Enthalpy at condenser outlet, subcooled liquid
H(6) = H(5); % Enthalpy remain unchanged through expansion valve
H(7) = getFluidProperty(libLoc, 'H', 'P', P(7), 'Q', 1, Fluid, 1, 1, 'MASS BASE SI'); % Enthalpy in evaporator, saturated vapor
...
for i = 1:length(P)
    T(i) = getFluidProperty(libLoc, 'T', 'P', P(i), 'H', H(i), Fluid, 1, 1, 'MASS BASE SI'); % Temperature at each state
    S(i) = getFluidProperty(libLoc, 'S', 'P', P(i), 'H', H(i), Fluid, 1, 1, 'MASS BASE SI'); % Entropy at each state
end
...
Q(6) = getFluidProperty(libLoc, 'QMASS', 'P', P(6), 'H', H(6), Fluid, 1, 1, 'MASS BASE SI'); % Quality at mixture state
